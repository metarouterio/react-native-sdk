...
describe("IdentityManager", () => {
  ...
  it("initializes anonymousId from storage", async () => {
    await manager.init();
    expect(getIdentityField).toHaveBeenCalledWith(
      identityStorage.ANONYMOUS_ID_KEY
    );
  ...
  it("sets and gets userId", async () => {
    ...
    expect(setIdentityField).toHaveBeenCalledWith(
      identityStorage.USER_ID_KEY,
      "user-456"
    );
  });
  ...
  it("resets internal state", async () => {
    ...
    expect(removeIdentityField).toHaveBeenCalledWith(
      identityStorage.ANONYMOUS_ID_KEY
    );
    expect(removeIdentityField).toHaveBeenCalledWith(
      identityStorage.USER_ID_KEY
